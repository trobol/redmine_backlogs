/**************************************
  EPIC
***************************************/
RB.Epic = RB.Object.create(RB.Issue, RB.EditableInplace, {
  ...
  editorDisplayed: function(editor){
    var tracker = editor.find('.epic_tracker_id.editor');
  ...
  setAllowedStatuses: function(tracker, status) {
    ...
    if (!status_id) { status_id = RB.constants.epic_states['default']; }

    var states = RB.constants.epic_states['transitions'][tracker_id][user_status][status_id];
    if (!states) { states = RB.constants.epic_states['transitions'][tracker_id][user_status][RB.constants.epic_states['transitions'][tracker_id][user_status]['default']]; }
    ...
    for (var i = 0; i < states.length; i++) {
      option = RB.$('<option/>');
      state = RB.constants.epic_states['states'][states[i]];
